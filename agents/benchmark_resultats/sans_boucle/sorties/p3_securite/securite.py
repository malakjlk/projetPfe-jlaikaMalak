...
# ── generer_rapport (score 50.6%, 1 itération(s)) ──
from subprocess import run
from typing import Optional

def generer_rapport(commande: str) -> str:
    try:
        sortie = run(["report_tool", commande], shell=False, capture_output=True, text=True, timeout=5).stdout
        return sortie
    except Exception as e:
...
def chercher_produit(nom: str):
    try:
        produits = Produit.__table__.select().where(Produit.__table__.c.nom.like(f"%{nom}%")).execute().fetchall()
        return produits
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
